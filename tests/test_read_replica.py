...
def sync_replica(primary_session, replica_session):
    ...
    In production this is handled automatically by PostgreSQL WAL
    streaming.
    """
    primary_rows = primary_session.query(Code).all()
    replica_session.query(Code).delete()
    for row in primary_rows:
        replica_session.merge(
            Code(
                id=row.id,
...
@pytest.fixture
def fake_cache():
    """Fake Redis cache to isolate DB routing tests from cache."""
...
def client_with_replica(setup_databases, fake_cache):
    ...
    with patch.object(
        main_module, "web_engine", primary_test_engine
    ), patch.object(
        main_module, "web_replica_engine", replica_test_engine
    ), patch.object(
        main_module, "r", fake_cache
    ), patch(
        "app.main.get_session", get_primary_session
    ), patch(
        "app.main.get_replica_session", get_replica_session
    ):
...
class TestReplicaSetup:
    ...
    def test_primary_and_replica_are_independent(
        self, primary_session, replica_session
    ):
        """Primary and replica should be separate databases."""
        url = Code(
            short_code_chars="abc1234567",
        ...
        replica_row = (
            replica_session.query(Code)
            .filter_by(short_code_chars="abc1234567")
            .one_or_none()
        )
        assert replica_row is None

    def test_replica_receives_data_after_sync(
        self, primary_session, replica_session
    ):
...
class TestWritesGoToPrimary:
    ...
    def test_new_url_written_to_primary(
        self, primary_session, replica_session
    ):
        url = Code(
            short_code_chars="write12345",
        ...
        primary_row = (
            primary_session.query(Code)
            .filter_by(short_code_chars="write12345")
            .one_or_none()
        )
        assert primary_row is not None

    def test_write_not_visible_on_replica_before_sync(
        self, primary_session, replica_session
    ):
        url = Code(
            short_code_chars="lag1234567",
            original_url="https://example.com",
            clicks=0,
        )
        primary_session.add(url)
        ...
        replica_row = (
            replica_session.query(Code)
            .filter_by(short_code_chars="lag1234567")
            .one_or_none()
        )
        assert replica_row is None

    def test_delete_goes_to_primary(
        self, primary_session, replica_session
    ):
        url = Code(
            short_code_chars="del1234567",
            original_url="https://example.com",
            clicks=0,
        )
        primary_session.add(url)
        ...
        primary_session.delete(
            primary_session.query(Code)
            .filter_by(short_code_chars="del1234567")
            .one()
        )
        primary_session.commit()
    ...
    def test_delete_propagates_to_replica_after_sync(
        self, primary_session, replica_session
    ):
        url = Code(
            short_code_chars="del1234567",
...
class TestReadsGoToReplica:
    ...
    def test_redirect_lookup_served_from_replica(
        self, primary_session, replica_session
    ):
        url = Code(
            short_code_chars="read123456",
            original_url="https://google.com",
            clicks=0,
        )
        primary_session.add(url)
        ...
        replica_row = (
            replica_session.query(Code)
            .filter_by(short_code_chars="read123456")
            .one_or_none()
        )
        assert replica_row is not None
        assert replica_row.original_url == "https://google.com"

    def test_stats_lookup_served_from_replica(
        self, primary_session, replica_session
    ):
        url = Code(
            short_code_chars="stat123456",
            original_url="https://example.com",
            clicks=42,
        )
        primary_session.add(url)
        ...
        replica_row = (
            replica_session.query(Code)
            .filter_by(short_code_chars="stat123456")
            .one_or_none()
        )
        assert replica_row is not None
        assert replica_row.clicks == 42

    def test_replica_returns_none_for_missing_key(
        self, primary_session, replica_session
    ):
        result = (
            replica_session.query(Code)
            .filter_by(short_code_chars="notexists0")
            .one_or_none()
        )
...
class TestReplicationLag:
    ...
    def test_replica_eventually_consistent_after_sync(
        self, primary_session, replica_session
    ):
        url = Code(
            short_code_chars="eventual12",
            original_url="https://example.com",
            clicks=0,
        )
        primary_session.add(url)
        ...
        assert (
            replica_session.query(Code)
            .filter_by(short_code_chars="eventual12")
            .one_or_none()
        ) is None
    ...
    def test_replica_reflects_click_count_updates_after_sync(
        self, primary_session, replica_session
    ):
        url = Code(
            short_code_chars="clicks1234",
            original_url="https://example.com",
            clicks=0,
        )
        primary_session.add(url)
        ...
        row = (
            primary_session.query(Code)
            .filter_by(short_code_chars="clicks1234")
            .one()
        )
        row.clicks = 10
        primary_session.commit()
    ...
    def test_multiple_writes_all_replicate(
        self, primary_session, replica_session
    ):
        for i in range(10):
            url = Code(
                short_code_chars=f"multi{i:05d}",
        ...
        for i in range(10):
            row = (
                replica_session.query(Code)
                .filter_by(short_code_chars=f"multi{i:05d}")
                .one_or_none()
            )
...
class TestPrimaryReplicaIsolation:
    ...
    def test_replica_read_does_not_affect_primary(
        self, primary_session, replica_session
    ):
        url = Code(
            short_code_chars="iso1234567",
            original_url="https://example.com",
            clicks=5,
        )
        primary_session.add(url)
        ...
        _ = (
            replica_session.query(Code)
            .filter_by(short_code_chars="iso1234567")
            .one()
        )
    ...
    def test_primary_write_does_not_affect_replica_until_sync(
        self, primary_session, replica_session
    ):
        url = Code(
            short_code_chars="iso2345678",
            original_url="https://example.com",
            clicks=0,
        )
        primary_session.add(url)
        ...
        assert (
            replica_session.query(Code)
            .filter_by(short_code_chars="iso2345678")
            .one_or_none()
        ) is None
        sync_replica(primary_session, replica_session)
        assert (
...
class TestAppWritesToPrimary:
    ...
    def test_shorten_writes_to_primary_not_replica(
        self, client_with_replica, primary_session, replica_session
    ):
        """POST /shorten should create a row on primary only."""
        response = client_with_replica.post(
            "/shorten",
    ...
    def test_delete_removes_from_primary(
        self, client_with_replica, primary_session, replica_session
    ):
        """DELETE /{short_code} should remove from primary."""
        url = Code(
            short_code_chars="delroute12",
            original_url="https://example.com",
            clicks=0,
        )
        primary_session.add(url)
        ...
        assert (
            primary_session.query(Code)
            .filter_by(short_code_chars="delroute12")
            .one_or_none()
        ) is None
...
class TestAppReadsFromReplica:
    ...
    def test_redirect_uses_replica_on_cache_miss(
        self, client_with_replica, primary_session, replica_session
    ):
        """GET /{short_code} cache miss should fall back to replica."""
        url = Code(
            short_code_chars="replicard12",
            original_url="https://google.com",
            clicks=0,
        )
        primary_session.add(url)
        ...
        response = client_with_replica.get(
            "/replicard12", follow_redirects=False
        )
        assert response.status_code == 302
        assert response.headers["location"] == "https://google.com"

    def test_redirect_returns_404_if_not_on_replica(
        self, client_with_replica, primary_session, replica_session
    ):
        """GET /{short_code} should 404 if URL exists on primary but
        not yet replicated."""
        url = Code(
            short_code_chars="norepl12345",
            original_url="https://example.com",
            clicks=0,
        )
        primary_session.add(url)
        ...
        response = client_with_replica.get(
            "/norepl12345", follow_redirects=False
        )
        assert response.status_code == 404

    def test_stats_uses_replica(
        self, client_with_replica, primary_session, replica_session
    ):
        """GET /stats/{short_code} should read from replica."""
        url = Code(
            short_code_chars="statsrep123",
            original_url="https://example.com",
            clicks=7,
        )
        primary_session.add(url)
    ...
    def test_stats_returns_404_if_not_on_replica(
        self, client_with_replica, primary_session, replica_session
    ):
        """GET /stats/{short_code} should 404 if URL not yet
        replicated."""
        url = Code(
            short_code_chars="statsnorep1",
            original_url="https://example.com",
            clicks=0,
        )
        primary_session.add(url)
